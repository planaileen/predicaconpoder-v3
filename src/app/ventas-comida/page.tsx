...
export default function VentasComidaPage() {
  ...
  const normalizarTelefono = (raw: string): string => {
    ...
    if (digits.length === 9) {
      // XXXXXXX999 -> 593XXXXXXX999 (por si viene sin 0 inicial)
      return "593" + digits;
    }
    return digits;
  };
  ...
  const getProductoTexto = () => {
    if (producto === "Otro") {
      return productoOtro.trim() || "tu pedido";
    }
    return producto;
  };
  ...
  const cargarContactos = async () => {
    try {
      ...
      const lista = snap.docs.map((d) => d.data() as Contacto);
      setContactos(lista);
    } catch (err) {
  ...
  // ===============================
  // IMPORTAR CSV (CON CABECERA nombre,telefono)
  // ===============================
  ...
  reader.onload = async ({ target }) => {
    try {
      ...
      // Procesar cabeceras de forma flexible
 ...
 useEffect(() => {
  const cargar = async () => {
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        // 🔥 MAPEADO CORRECTO DEL TIPO Cliente
        const mapeados: Cliente[] = rawClientes.map((c: any) => ({
          ...
          telefono: c.telefono ?? "",
          producto: c.producto ?? getProductoTexto(), // ← OBLIGATORIO
        }));

        setClientes(mapeados);
      } else {
        // Si no existe documento, limpiar valores
        setInversion("");
        setCostoUnitario("");
        setProducto("Morocho");
        setProductoOtro("");
  ...
  // CÁLCULOS
  // ===============================
  const totalVendido = clientes.reduce((acc, c) => acc + c.total, 0);
  const ganancia = totalVendido - (Number(inversion) || 0);

  const comidasPosibles =
    Number(inversion) > 0 && Number(costoUnitario) > 0
      ? Math.floor(Number(inversion) / Number(costoUnitario))
      : 0;

  const comidasVendidas = clientes.reduce((acc, c) => acc + c.cantidad, 0);
  const comidasRestantes = comidasPosibles - comidasVendidas;
  ...
  // ===============================
  // SELECCIONAR CONTACTO DEL SELECT
  // ===============================
  ...
  const guardarCliente = async () => {
    ...
    if (!telefono.trim()) return alert("Ingrese el número de WhatsApp del cliente");
    ...
    if (editId !== null) {
      ...
      nuevos = clientes.map((c) =>
        c.id === editId
          ? { ...c, nombre, cantidad: cant, total, telefono: telNorm, producto: prodTexto }
          : c
      );
      setEditId(null);
    } else {
  ...
  const editarCliente = (cliente: Cliente) => {
    ...
    setContactoSeleccionado(""); // para que se vea claro que se está editando manual
  };
  ...
  const generarReportePDF = () => {
    ...
    const textoProducto = getProductoTexto();
    ...
    win.document.write(`
      ...
      <body>
        ...
        <p><strong>Producto del día:</strong> ${textoProducto}</p>
        <p><strong>Comidas posibles:</strong> ${comidasPosibles}</p>
        <p><strong>Comidas vendidas:</strong> ${comidasVendidas}</p>
        <p><strong>Comidas restantes:</strong> ${comidasRestantes}</p>
          ...
          <tbody>
            ${clientes
              ...
              <tr>
                ...
                <td>${c.entregado ? "✔ Entregado" : "Pendiente"}</td>
              </tr>`
              )
              .join("")}
          </tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="bg-neutral-900 p-4 rounded-2xl border border-neutral-800">
                ...
                <p>
                  <strong>Comidas posibles:</strong>{" "}
                  <span className="text-indigo-400">{comidasPosibles}</span>
                </p>

                <p>
                  <strong>Comidas vendidas:</strong>{" "}
                  <span className="text-teal-400">{comidasVendidas}</span>
                </p>

                <p>
                  <strong>Comidas restantes:</strong>{" "}
                  <span
                    className={
                      comidasRestantes >= 0
                        ? "text-green-400"
